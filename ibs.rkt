...
(define-struct automaton (init-claim ff fo of oo))

(define all-f (make-automaton 1 1 1 1 1))
(define all-o (make-automaton 0 0 0 0 0))
...
(define (next-claim automaton previous-claims)
  (let ([look-up
         (cond
          [(equal? previous-claims '(1 1)) automaton-ff]
          [(equal? previous-claims '(1 0)) automaton-fo]
          [(equal? previous-claims '(0 1)) automaton-of]
          [(equal? previous-claims '(0 0)) automaton-oo])])
    (look-up automaton)))

(define (match-claims claims)
  (cond [(equal? claims '(1 1)) (list 2 1)]
        [(equal? claims '(1 0)) (list 0 0)]
        [(equal? claims '(0 1)) (list 0 0)]
        [(equal? claims '(0 0)) (list 1 2)]))
...
(define (top-identify t an-identified-hash)
  (let* ([top-list (map car (take (rank an-identified-hash) t))]
         [l (length top-list)])
    (for/list ([i l])
      (eval
       (list 'define (n->xn i)
